...
class TodoListModel:
    ...
    def __init__(self, database_name: str = "todo_list") -> None:
        self.database_name = database_name

        self.database_path = os.path.join(Assets().todo_list_database_path, database_name + ".db")
    ...
    def load_tasks(self) -> dict[int, Task]:
        with self.conn:
            ...
            return {id_: Task(id_, description, pickle.loads(importance), idx)
                    for id_, description, importance, idx in rows}

    def add_task(self, task: Task) -> None:
        self.tasks[task.id] = task
        with self.conn:
            self.cursor.execute(
                f"INSERT INTO '{self.database_name}' VALUES (?, ?, ?, ?)",
                (task.id, task.description, pickle.dumps(task.importance), task.idx)
            )
