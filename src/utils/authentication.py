...
class GoogleAuthentication:
    ...
    def authenticate_new_user() -> User:
        flow = InstalledAppFlow.from_client_secrets_file(
            Assets().google_credentials_file_path,
    ...
    @staticmethod
    def authenticate_user_with_token(email: str) -> User:
    ...
    def authenticate_user(email: str):
        ...
        return creds.build("calendar", "v3", creds)

    @staticmethod
    def initialize_service(email: str):
        token_file = os.path.join(Assets().google_tokens_path, f"token_{email}.json")
        creds = Credentials.from_authorized_user_file(token_file, SCOPES)
        return creds.build("calendar", "v3", creds)
